...
Snipe.menu = function(producer, callback, menu_context)
	...
	local function open()
		...
		if #meta ~= #items then
			vim.notify(
				"(snipe) Meta-data length from producer does not match the number of items",
		...
		state.page_index = state.page_index > page_count and page_count -- clamp
			or state.page_index
...
Snipe.create_buffer_menu_toggler = function(bopts_)
	...
	return Snipe.create_menu_toggler(function()
		return Snipe.buffer_producer(bopts)
	end, function(bufnr, _)
